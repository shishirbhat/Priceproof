...
scraperHealthRouter.get(
  ...
  ah(async (_req, res) => {
    ...
    select c.*, p.name as portal_name,
...
// Per field, coverage over the last N collections for a portal — a field's
...
scraperHealthRouter.get(
  ...
  ah(async (_req, res) => {
    const { rows } = await pool.query(`
    select fc.*, c.triggered_at, c.portal_id, p.name as portal_name
